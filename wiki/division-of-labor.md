...
| Adding a source via `/ctx-extract` | User triggers; LLM executes |
...
- Is there a use case for the user editing a wiki page directly? Current default: no — edit the raw source or use `/ctx-query` to file a new claim. But there might be edge cases (typo fix, sensitive redaction) where direct edit is fine.
