...
- `/ctx-visualize` is **mechanical** — it renders structure that already exists (the `[[link]]` graph, mermaid blocks, slides). No reasoning about a query.
- `/ctx-diagram` is **semantic** — it composes a *new* diagram that answers an intent by synthesizing across pages.
...
- **Wiki-only by default — no web search, no promotion.** A diagram is a *view* of existing knowledge. If the wiki lacks what the intent needs, say so and recommend running `/ctx-query` first to fill the gap, then re-running this.
...
### 2. Retrieve (reuse `/ctx-query` discipline)
...
If retrieval comes back thin (the wiki doesn't cover the intent), stop and tell the user, recommending `/ctx-query "<intent>"` to fill the gap first.
