...
If `scripts/profile-resolve.py` exists, resolve the optional profile with
`python3 scripts/profile-resolve.py --root . --json`. Resolution failure is a
setup error. An inactive profile leaves all generic checks below unchanged. For
active `client-decision`, additionally run:
...
Report claim/provenance failures plus citation resolution, valid-time coverage,
freshness, contradictions, stale assumptions, unsupported claims, unknown owners, and
review items. These remain report-only with `--apply`: do not auto-resolve attribution,
contradiction, or possible supersession, and do not invent an aggregate AI score.

If the resolver script is absent, continue with the generic checks only; older
no-profile installs remain valid.
