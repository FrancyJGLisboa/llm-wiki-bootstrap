...
- **Verification:** `verify-multi-wiki.sh` green on M1–M3 (deterministic scaffold+register, enumerate+drift, escape-hatch+refuse-clobber). M4–M5 (seeded schema-validity + domain relevance) green on a real end-to-end `/wiki-new "vineyard-ops"` run, and the oracle correctly reds intentional defects (broken link, unlinked seed, provenance gap).
