...
# (C1–C5), and the regression guards (R1–R17) into a single exit-code-
# driven test.
#
# Exit 0 iff all 22 checks pass.
#
# --no-build : skip the LLM build phase (which needs the `claude` CLI) and run
#   only the 22 deterministic checks (C1–C5 asserts on the committed artifacts +
#   R1–R17 guards). This is the CI path — the build phase is a precondition that
...
# ──── REGRESSION GUARDS R1–R17 ────
section "Regression guards (R1–R17)"
